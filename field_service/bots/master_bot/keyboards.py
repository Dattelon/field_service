...
def start_onboarding_keyboard() -> InlineKeyboardMarkup:
    return inline_keyboard(
        [[InlineKeyboardButton(text="Начать онбординг", callback_data="m:onboarding:start")]]
    )
...
def vehicle_keyboard() -> InlineKeyboardMarkup:
    return inline_keyboard(
        [
            [InlineKeyboardButton(text="Да, есть авто", callback_data="m:onboarding:vehicle_yes")],
            [InlineKeyboardButton(text="Нет, без авто", callback_data="m:onboarding:vehicle_no")],
        ]
    )
...
def main_menu_keyboard(master: m.masters) -> InlineKeyboardMarkup:
    ...
    if getattr(master, "verified", False):
        ...
        if shift_status is m.ShiftStatus.SHIFT_OFF:
            rows.append([InlineKeyboardButton(text="Включить смену", callback_data="m:sh:on")])
        elif shift_status is m.ShiftStatus.SHIFT_ON:
            rows.append([InlineKeyboardButton(text="Перерыв 2 часа", callback_data="m:sh:brk")])
            rows.append([InlineKeyboardButton(text="Выключить смену", callback_data="m:sh:off")])
        elif shift_status is m.ShiftStatus.BREAK:
            rows.append([InlineKeyboardButton(text="Вернуться к смене", callback_data="m:sh:brk:ok")])
            rows.append([InlineKeyboardButton(text="Выключить смену", callback_data="m:sh:off")])
        rows.append([InlineKeyboardButton(text="Новые офферы", callback_data="m:new")])
        rows.append([InlineKeyboardButton(text="Активный заказ", callback_data="m:act")])
        rows.append([InlineKeyboardButton(text="Финансы", callback_data="m:fin")])
        rows.append([InlineKeyboardButton(text="Реферальная программа", callback_data="m:rf")])
        rows.append([InlineKeyboardButton(text="База знаний", callback_data="m:kb")])
    else:
        rows.append([InlineKeyboardButton(text="Начать онбординг", callback_data="m:onboarding:start")])
    return inline_keyboard(rows)
...
def _method_title(method: m.PayoutMethod) -> str:
    mapping = {
        ...
        m.PayoutMethod.BANK_ACCOUNT: "Банковские реквизиты",
    }
