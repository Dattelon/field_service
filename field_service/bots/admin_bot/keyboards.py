...
def main_menu(staff: StaffUser) -> InlineKeyboardMarkup:
    ...
    if staff.role is StaffRole.GLOBAL_ADMIN:
        kb.button(text="🔐 Доступ/персонал", callback_data="adm:staff:menu")
...
def queue_list_keyboard(
    ...
    controls.button(text="🏠 В меню", callback_data="adm:menu")
    controls.adjust(2)
    kb.attach(controls)
    return kb.as_markup()
...
def order_card_keyboard(
    ...
    *,
    ...
    if allow_return:
        actions.button(text='🔄 Вернуть в поиск', callback_data=f'adm:q:ret:{order_id}')
    if allow_cancel:
...
def assign_menu_keyboard(order_id: int) -> InlineKeyboardMarkup:
    ...
    kb.button(text='🧑‍🔧 Выбрать вручную', callback_data=f'adm:q:as:man:{order_id}:1')
...
def finance_card_actions(detail: CommissionDetail, segment: str, page: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="🧾 Открыть чеки", callback_data=f"adm:f:cm:open:{detail.id}")
    ...
    if detail.master_id is not None:
        kb.button(text="🚫 Блокировать", callback_data=f"adm:f:cm:blk:{detail.id}")
...
def owner_pay_actions_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.button(text="📣 Разослать реквизиты", callback_data="adm:f:set:bc")
    kb.button(text="◀️ Назад", callback_data="adm:f")
    kb.adjust(2, 1)
    return kb.as_markup()
...
def new_order_street_keyboard(streets: Sequence[tuple[int, str]]) -> InlineKeyboardMarkup:
    ...
    kb.button(text="🔎 Поиск ещё раз", callback_data="adm:new:street:search_again")
...
def new_order_attachments_keyboard(has_any: bool) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="📎 Добавить файл", callback_data="adm:new:att:add")
...
def new_order_asap_late_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.button(text="Выбрать другой слот", callback_data="adm:new:slot:reslot")
    kb.adjust(1)
    return kb.as_markup()
...
def settings_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.button(text="🗺 География", callback_data="adm:s:group:geo")
