...
class cities(Base):
    ...
    districts: Mapped[list["districts"]] = relationship(back_populates="city", cascade="all, delete-orphan")
...
class districts(Base):
    ...
    streets: Mapped[list["streets"]] = relationship(back_populates="district", cascade="all, delete-orphan")
...
class streets(Base):
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    city_id: Mapped[int] = mapped_column(ForeignKey("cities.id", ondelete="CASCADE"), nullable=False, index=True)
    district_id: Mapped[Optional[int]] = mapped_column(ForeignKey("districts.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    __table_args__ = (
        UniqueConstraint("city_id", "district_id", "name", name="uq_streets__city_district_name"),
    )
...
class masters(Base):
    ...
    rating: Mapped[float] = mapped_column(Float, nullable=False, default=5.0, server_default="5.0")
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
    ...
    referral_code: Mapped[Optional[str]] = mapped_column(String(32), unique=True, index=True)
    referred_by_master_id: Mapped[Optional[int]] = mapped_column(ForeignKey("masters.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    payout_method: Mapped[Optional[PayoutMethod]] = mapped_column(Enum(PayoutMethod, name="payout_method"))
    payout_data: Mapped[Optional[dict]] = mapped_column(JSONB)
    has_vehicle: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
...
class staff_users(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
    commission_requisites: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict, server_default=text("'{}'::jsonb"))

class staff_cities(Base):
    staff_user_id: Mapped[int] = mapped_column(ForeignKey("staff_users.id", ondelete="CASCADE"), primary_key=True)
...
class orders(Base):
    ...
    district_id: Mapped[Optional[int]] = mapped_column(ForeignKey("districts.id", ondelete="SET NULL"), nullable=True, index=True)
    street_id: Mapped[Optional[int]] = mapped_column(ForeignKey("streets.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    status: Mapped[OrderStatus] = mapped_column(Enum(OrderStatus, name="order_status"), nullable=False, index=True)
    ...
    preferred_master_id: Mapped[Optional[int]] = mapped_column(ForeignKey("masters.id", ondelete="SET NULL"), nullable=True, index=True)  # приоритет для гарантий
...
class order_status_history(Base):
    ...
    changed_by_staff_id: Mapped[Optional[int]] = mapped_column(ForeignKey("staff_users.id", ondelete="SET NULL"))
    changed_by_master_id: Mapped[Optional[int]] = mapped_column(ForeignKey("masters.id", ondelete="SET NULL"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)
...
class offers(Base):
    ...
    round_number: Mapped[int] = mapped_column(SmallInteger, nullable=False, default=1, server_default="1")
    state: Mapped[OfferState] = mapped_column(Enum(OfferState, name="offer_state"), nullable=False, index=True)
...
class attachments(Base):
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    entity_type: Mapped[AttachmentEntity] = mapped_column(Enum(AttachmentEntity, name="attachment_entity"), nullable=False, index=True)
    entity_id: Mapped[int] = mapped_column(BigInteger, nullable=False, index=True)
    file_type: Mapped[AttachmentFileType] = mapped_column(Enum(AttachmentFileType, name="attachment_file_type"), nullable=False)
    ...
    uploaded_by_master_id: Mapped[Optional[int]] = mapped_column(ForeignKey("masters.id", ondelete="SET NULL"))
    uploaded_by_staff_id: Mapped[Optional[int]] = mapped_column(ForeignKey("staff_users.id", ondelete="SET NULL"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    __table_args__ = (
        Index("ix_attachments__etype_eid", "entity_type", "entity_id"),
    )
...
class commissions(Base):
    ...
    status: Mapped[CommissionStatus] = mapped_column(Enum(CommissionStatus, name="commission_status"), nullable=False, index=True)
    due_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, index=True)
    paid_reported_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    paid_approved_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    ...
    has_checks: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    pay_to_snapshot: Mapped[Optional[dict]] = mapped_column(JSONB)
    blocked_applied: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    blocked_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    ...
    __table_args__ = (
        Index("ix_commissions__status_due", "status", "due_at"),
        Index("ix_commissions__master_status", "master_id", "status"),
        Index("ix_commissions__ispaid_due", "is_paid", "due_at"),
    )
# ===== Referrals =====

class referrals(Base):
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    master_id: Mapped[int] = mapped_column(ForeignKey("masters.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)  # приглашённый
    referrer_id: Mapped[int] = mapped_column(ForeignKey("masters.id", ondelete="CASCADE"), nullable=False, index=True)            # пригласивший (L1)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

class referral_rewards(Base):
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    referrer_id: Mapped[int] = mapped_column(ForeignKey("masters.id", ondelete="CASCADE"), nullable=False, index=True)
    referred_master_id: Mapped[int] = mapped_column(ForeignKey("masters.id", ondelete="CASCADE"), nullable=False, index=True)
    commission_id: Mapped[int] = mapped_column(ForeignKey("commissions.id", ondelete="CASCADE"), nullable=False, index=True)
    level: Mapped[int] = mapped_column(SmallInteger, nullable=False)
    percent: Mapped[float] = mapped_column(Numeric(5, 2), nullable=False)
    amount: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)
    status: Mapped[ReferralRewardStatus] = mapped_column(Enum(ReferralRewardStatus, name="referral_reward_status"), nullable=False, index=True)
    paid_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    __table_args__ = (
        UniqueConstraint("referrer_id", "commission_id", "level", name="uq_referral_rewards__once_per_level"),
...
class settings(Base):
    ...
    value_type: Mapped[str] = mapped_column(String(16), nullable=False, default="STR", server_default="STR")  # INT/FLOAT/BOOL/STR/JSON/TIME
...
class master_skills(Base):
    master_id: Mapped[int] = mapped_column(ForeignKey("masters.id", ondelete="CASCADE"), primary_key=True)
