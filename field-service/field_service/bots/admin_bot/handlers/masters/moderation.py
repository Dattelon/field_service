...
async def moderation_card(cq: CallbackQuery, staff: StaffUser, state: FSMContext) -> None:
    ...
    try:
        ...
        await cq.answer("❌ Неверный формат действия", show_alert=True)
        return
...
async def moderation_approve(cq: CallbackQuery, staff: StaffUser) -> None:
    ...
    await admin_masters.notify_master(
        cq.bot,
        master_id,
        "✅ Ваша анкета одобрена. Теперь вам доступны заявки.",
    )
...
async def moderation_reject(cq: CallbackQuery, state: FSMContext, staff: StaffUser) -> None:
    ...
    if cq.message:
        await cq.message.answer("✍️ Введите причину отклонения анкеты (до 1200 символов).")
    await cq.answer()
...
async def disable_delete(cq: CallbackQuery, staff: StaffUser) -> None:
    await cq.answer("⚠️ Удаление из модерации недоступно", show_alert=True)
...
async def bulk_toggle_master(cq: CallbackQuery, staff: StaffUser, state: FSMContext) -> None:
    ...
    try:
        ...
        await cq.answer("❌ Ошибка разбора параметров", show_alert=True)
        return
    ...
    if master_id in selected:
        ...
        action_text = "Снят с выборки"
    else:
        selected.add(master_id)
        action_text = "Добавлен в выборку"
    ...
    if cq.message:
        await cq.message.edit_text(text, reply_markup=markup)
    await cq.answer(f"{action_text} для мастера #{master_id}")
...
async def bulk_clear_selection(cq: CallbackQuery, staff: StaffUser, state: FSMContext) -> None:
    ...
    if cq.message:
        await cq.message.edit_text(text, reply_markup=markup)
    await cq.answer("✅ Выборка очищена")
...
async def bulk_approve_ask_confirm(cq: CallbackQuery, staff: StaffUser, state: FSMContext) -> None:
    ...
    if not selected:
        await cq.answer("⚠️ Не выбрано ни одного мастера", show_alert=True)
        return
    ...
    if cq.message:
        await cq.message.edit_text(
            f"⚠️ <b>Массовое одобрение анкет</b>\n\n"
            f"Вы действительно хотите одобрить <b>{len(selected)}</b> мастеров?\n\n"
            f"Все они получат доступ к заявкам.",
...
async def bulk_approve_confirmed(cq: CallbackQuery, staff: StaffUser, state: FSMContext) -> None:
    ...
    if cq.message:
        await cq.message.edit_text(f"⏳ Обработка {len(selected)} мастеров...")
...
async def bulk_reject_ask_reason(cq: CallbackQuery, staff: StaffUser, state: FSMContext) -> None:
    ...
    if cq.message:
        await cq.message.answer(
            f"✍️ Введите причину отклонения для {len(selected)} мастеров (до 1200 символов):"
        )
    await cq.answer()
