...
def upgrade() -> None:
    ...
    op.add_column("commissions", sa.Column("paid_reported_at", sa.DateTime(timezone=True)))
    op.add_column("commissions", sa.Column("paid_approved_at", sa.DateTime(timezone=True)))
    op.add_column("commissions", sa.Column("paid_amount", sa.Numeric(10,2)))
    op.add_column("commissions", sa.Column("is_paid", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("commissions", sa.Column("has_checks", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("commissions", sa.Column("pay_to_snapshot", postgresql.JSONB(astext_type=sa.Text()), nullable=True))
