...
def upgrade():
    ...
    op.create_index("ix_master_districts__district", "master_districts", ["district_id"], unique=False)
    # под фильтр по категории после добавления orders.skill_id
    op.create_index("ix_master_skills__skill", "master_skills", ["skill_id"], unique=False)
