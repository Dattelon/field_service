...
def upgrade():
    ...
    if "ix_staff_users__tg_user_id" not in existing:
        op.create_index("ix_staff_users__tg_user_id", "staff_users", ["tg_user_id"], unique=False)

    # 2) Персональный лимит активных (опционально, для adm:m:lim)
    op.add_column("masters", sa.Column("max_active_orders_override", sa.SmallInteger(), nullable=True))

    # 3) Коды доступа персонала
    staff_role = postgresql.ENUM("ADMIN","LOGIST", name="staff_role", create_type=False)
    op.create_table(
        "staff_access_codes",
        ...
        sa.Column("used_by_staff_id", sa.Integer, sa.ForeignKey("staff_users.id", ondelete="SET NULL")),
        sa.Column("expires_at", sa.DateTime(timezone=True)),
        sa.Column("is_revoked", sa.Boolean, nullable=False, server_default=sa.text("false")),
        sa.Column("used_at", sa.DateTime(timezone=True)),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
    )
    op.create_index("ix_staff_access_codes__code", "staff_access_codes", ["code"], unique=True)
